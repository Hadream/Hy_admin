        ...
        <Card :bordered="false" dis-hover class="ivu-mt">
            <Form ref="formValidate" :model="formValidate" :label-width="labelWidth" :label-position="labelPosition" class="tabform" @submit.native.prevent>
                <Row :gutter="24" type="flex">
                    ...
                    <Col span="24">
                        <FormItem label="搜索：">
                            <Input search enter-button @on-search="selChange" placeholder="请输入商品名称/ID" element-id="name"
                                   v-model="formValidate.kerword" style="width: 30%;"/>
                        </FormItem>
                    </Col>
                </Row>
                <Row type="flex">
            ...
            <Table :columns="columns1" :data="tabList" ref="table" class="mt25"
                ...
                <template slot-scope="{ row, index }" slot="is_mer_show">
                    <i-switch v-model="row.is_show" :value="row.is_show" :true-value="1" :false-value="0"
                              @on-change="onchangeIsShow(row)" size="large"  :disabled="row.audit_status != 2">
